...
function arslanLog(message, type = 'info') {
    ...
    console.log(`${icons[type] || '📝'} [𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗] ${new Date().toISOString()}: ${message}`);
}
    ...
    try {
        ...
        const conn = makeWASocket({
            ...
            getMessage: async (key) => {
                ...
                return msg && msg.message ? msg.message : { conversation: '𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗' };
            }
        });
            ...
            if (connection === 'open') {
                ...
                if (!existingSession) {
                    await conn.sendMessage(userJid, {
                        ...
                        caption: `\n╭────────────────────◇\n│✦ *SHAMIIXMRASAD-MD-MINI — CONNECTED* 🔥\n│✦ Type *${prefix}menu* to see all commands 💫\n│✦ Prefix 『 ${prefix} 』  Mode 〔${mode}〕\n╰────────────────────○\n*© Powered by 𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗*`
                    });
                }
            }
            if (connection === 'close') {
        ...
        arslanLog(`𝗦𝗛𝗔𝗠𝗜𝗜 𝗫 𝗠𝗥𝗔𝗦𝗔𝗗 𝗠𝗗 Pair error: ${err.message}`, 'error');
...
router.get('/update-config', async (req, res) => {
    ...
    try {
        await socket.sendMessage(jidNormalizedUser(socket.user.id), { text: `*🔐 SHAMIIXMRASAD-MD — CONFIG UPDATE*\n\nOTP: *${otp}*\nValid 5 minutes` });
